  ...
  _createClass(xhrUpload, [
    ...
    },
    {
      ...
      value: function ajaxForm(data) {
        ...
        var type = Object.prototype.toString.call(this.option.data)
        if (type === '[object String]') {
          this.formData.append(this.option.data, data[0], data[0].name);
          this.xhr.send(this.formData);
          return
        }
        for (var i in this.option.data) {
          if (this.option.data[i]) {
            this.formData.append(i, this.option.data[i], '');
          }
          this.formData.append(i, data[0], data[0].name);
        }
        this.xhr.send(this.formData);
      }
    },
